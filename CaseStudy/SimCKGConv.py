...
from ckgconv.layer.utils.affine import (
    AffineTransformLayer,
    RecenterAffineTransformLayer,
)
...
class CKGConv(nn.Module):
        ...
        **kwargs
        ...
        for i in range(num_blocks):
            blocks = blocks + [
                ...
                (act(), "h -> h"),
                (
                    nn.Linear(hid_dim, int(hid_dim * self.ffn_ratio), bias=use_bias),
                    "h -> h",
                ),
                (norm_fn(int(hid_dim * self.ffn_ratio)), "h -> h"),
                (act(), "h -> h"),
                (
                    nn.Linear(int(hid_dim * self.ffn_ratio), hid_dim, bias=use_bias),
                    "h -> h",
                ),
                (
                    (
    ...
    def propagate_attention(self, x, pe_index, pe_val, deg=None):
        ...
        wV = scatter(msg, edge_index[1], dim=0, dim_size=x.size(0), reduce=reduce)

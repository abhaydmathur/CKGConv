...
for i in range(num_layers):
    GCN += [
        (
            GCNConv(hid_dim, hid_dim, add_self_loops=True, normalize=True),
            "x, edge_index -> x",
        ),
        (nn.GELU(), "x -> x"),
    ]
GCN += [(nn.Linear(hid_dim, out_dim), "x -> x"), (nn.Sigmoid, "x -> x")]
...
for i in range(num_layers):
    CKGCN += [
        (
            CKGConv(hid_dim, hid_dim, pe_dim, ffn_ratio=1.0, num_blocks=1),
            "x, pe_index, pe_val -> x",
        ),
        (nn.GELU(), "x -> x"),
    ]
CKGCN += [(nn.Linear(hid_dim, out_dim), "x -> x"), (nn.Sigmoid, "x -> x")]

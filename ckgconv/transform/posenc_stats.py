...
from .rrwp import add_full_rrwp_new
from .spd import add_spd
from .rd import add_rd
...
def compute_posenc_stats(data, pe_types, is_undirected, cfg):
    ...
    if "RWSE" in pe_types:
        ...
        data.deg = pyg.utils.degree(
            data.edge_index[1], num_nodes=data.num_nodes, dtype=torch.float
        )
    ...
    if "HKdiagSE" in pe_types or "HKfullPE" in pe_types:
        ...
        if "HKdiagSE" in pe_types:
            ...
            hk_diag = get_heat_kernels_diag(
                evects_heat, evals_heat, kernel_times=kernel_param.times, space_dim=0
            )
            data.pestat_HKdiagSE = hk_diag
    ...
    if "RRWP" in pe_types:
        ...
        transform = partial(
            ...
            attr_name_rel="rrwp",
            add_identity=True,
            use_sym=param.get("use_sym", False),
        )
        data = transform(data)

    if "SPD" in pe_types:
        param = cfg.posenc_SPD
        pe_func = add_spd
...
class ComputePosencStat(BaseTransform):
    ...
    def __call__(self, data: Data) -> Data:
        data = compute_posenc_stats(
            data, pe_types=self.pe_types, is_undirected=self.is_undirected, cfg=self.cfg
        )
        return data

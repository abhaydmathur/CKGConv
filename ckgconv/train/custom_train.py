...
from ckgconv.loss.subtoken_prediction_loss import subtoken_cross_entropy
from ckgconv.utils import cfg_to_dict, flatten_dict, make_wandb_name, mlflow_log_cfgdict
import warnings
...
def custom_train(loggers, loaders, model, optimizer, scheduler):
    ...
    cfg.vp_momentum = min(1.0, cfg.vp_momentum)
    if cfg.vp_momentum < 1:
        assert cfg.vp_momentum > 0
        mo_vp_loss, mo_vp_metric = [], []
    ...
    if cfg.mlflow.use:
        ...
        if cfg.get("cfg_file", None) is not None:
            mlflow.log_artifact(cfg.cfg_file)  # log the whole config-file
    ...
    for cur_epoch in range(start_epoch, cfg.optim.max_epoch):
        ...
        if "timm" in cfg.optim.scheduler:
            scheduler.step(epoch=cur_epoch + 1, metric=val_perf[-1]["loss"])
        elif (
            cfg.optim.scheduler == "reduce_on_plateau" or "timm" in cfg.optim.scheduler
        ):
        ...
        if is_eval_epoch(cur_epoch):
            ...
            # --------  momemtum update val_perf -----------
            if cfg.vp_momentum < 1:
                cur_vp_loss = val_perf[-1]["loss"]
                if len(mo_vp_loss) > 0:
                    mo_vp_loss.append(
                        mo_vp_loss[-1] * (1 - cfg.vp_momentum)
                        + cur_vp_loss * cfg.vp_momentum
                    )
                else:
                    mo_vp_loss.append(cur_vp_loss)

                best_epoch = np.array(mo_vp_loss).argmin()
                if cfg.mlflow.use:
                    mlflow.log_metric("val/mo_loss", mo_vp_loss[-1], step=cur_epoch)
            # -----------------------------------------------
            ...
            if cfg.metric_best != "auto":
                ...
                # --------  momemtum update val_perf -----------
                if cfg.vp_momentum < 1:
                    cur_vp_metric = val_perf[-1][m]
                    if len(mo_vp_metric) > 0:
                        mo_vp_metric.append(
                            mo_vp_metric[-1] * (1 - cfg.vp_momentum)
                            + cur_vp_metric * cfg.vp_momentum
                        )
                    else:
                        mo_vp_metric.append(cur_vp_metric)

                    best_epoch = getattr(np.array(mo_vp_metric), cfg.metric_agg)()
                    if cfg.mlflow.use:
                        mlflow.log_metric(
                            f"val/mo_{m}", mo_vp_metric[-1], step=cur_epoch
                        )
                # -----------------------------------------------
                ...
                if cfg.wandb.use or cfg.mlflow.use:
                    ...
                    for i, s in enumerate(["train", "val", "test"]):
                        ...
                        if m in perf[i][best_epoch]:
                            ...
                            if cfg.mlflow.use:
                                mlflow.log_metric(
                                    f"best_{s}_perf", perf[i][best_epoch][m]
                                )
                    ...
                    if cfg.mlflow.use:
                        ...
                        mlflow.log_metric(
                            "full_epoch_time_avg", np.mean(full_epoch_times)
                        )
                        mlflow.log_metric(
                            "full_epoch_time_sum", np.sum(full_epoch_times)
                        )
...
def log_attn_weights(loggers, loaders, model, optimizer=None, scheduler=None):
    ...
    from ckgconv.utils import unbatch, unbatch_edge_index
...
@register_train("log-conv-kernel")
def log_conv_weights(loggers, loaders, model, optimizer=None, scheduler=None):
    """
    Customized pipeline to inference on the test set and log the kernel
    weights in CKGConv modules.
    ...
    for b_index, batch in enumerate(loader):
        ...
        X_orig = unbatch(batch.x.cpu(), batch.batch.cpu())
        edge_orig = unbatch_edge_index(batch.edge_index.cpu(), batch.batch.cpu())
        Y_orig = batch.y.cpu()
        batch.to(torch.device(cfg.device))
        model.eval()
        model(batch)
        edge_index_ = batch.edge_index
        batch_ind_ = batch.batch
        Y_pred = batch.graph_feature
            ...
            graphs.append(
                {
                    ...
                    "y_orig": Y_orig[i],
                    "y_pred": Y_pred[i],
                    "edge_index_orig": edge_orig[i],
                    "x_final": X[i],
                    "edge_index": edge_indices[i],
                    "kernel_val": [],  # List with attn weights in layers from 0 to L-1.
                }
            )
        ...
        for l_i, (name, module) in enumerate(model.model.layers.named_children()):
            ...
            if hasattr(conv_layer, "kernel_val"):
                print(l_i, name, conv_layer.kernel_val.shape)
                for g_i in range(bsize):
                    # Clip to the number of nodes in this graph.
                    # num_nodes = graphs[g_i]['num_nodes']
                    # aw = module.attn_weights[g_i, :num_nodes, :num_nodes]
                    mask = (batch_ind_ == g_i)[edge_index_[1]]
                    aw = conv_layer.kernel_val[mask]
                    graphs[g_i]["kernel_val"].append(aw.cpu())
        output += graphs

    logging.info(
        f"[*] Collected a total of {len(output)} graphs and their "
        f"attention weights for {len(output[0]['kernel_val'])} layers."
    )

    # Save the graphs and their attention stats.
    save_file = osp.join(cfg.run_dir, "graph_kernel_stats+pred+label.pt")
    logging.info(f"Saving to file: {save_file}")
    torch.save(output, save_file)

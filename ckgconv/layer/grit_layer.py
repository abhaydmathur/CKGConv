...
class MultiHeadAttentionLayerGritSparse(nn.Module):
    ...
    def propagate_attention(self, batch):
        ...
        if batch.get("E", None) is not None:
            ...
            elif self.signed_sqrt:
                score = score * E_w
                score = torch.sqrt(torch.relu(score)) - torch.sqrt(torch.relu(-score))
                score = score + E_b
            elif self.sqrt_relu:
                score = torch.sqrt(torch.relu(score * E_w)) + E_b
            else:
                score = score * E_w + E_b
    ...
    def forward(self, batch):
        ...
        if batch.get("edge_attr", None) is not None:
            ...
            if self.ure:
                batch.Ure = self.URE(batch.edge_attr).view(-1, self.num_heads, 1)
        else:
            batch.E = None
...
class GritTransformerLayer(nn.Module):
    ...
    def forward(self, batch):
        ...
        # if not self.sigmoid_deg:
        #     xxxxx
        # else:
        #     h = h + torch.sigmoid(self.deg_coef) * log_deg * h
        h = self.O_h(h)
...
class PreNormGritTransformerLayer(GritTransformerLayer):
    ...
    def forward(self, batch):
        ...
        h_in1 = h  # for first residual connection
        e = e_in1 = batch.get("edge_attr", None)

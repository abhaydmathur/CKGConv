...
def trunc_xavier_normal_init_(m):
    if isinstance(m, (nn.Linear, nn.Conv1d)):
        variance_scaling_(
            m.weight, scale=1.0, mode="fan_avg", distribution="truncated_normal"
        )
    elif isinstance(m, nn.Parameter):
        variance_scaling_(m, scale=1.0, mode="fan_avg", distribution="truncated_normal")


def trunc_kaiming_normal_init_(m, scale=1.0):
    constant = 0.87962566103423978
    if isinstance(m, (nn.Linear, nn.Conv1d)):

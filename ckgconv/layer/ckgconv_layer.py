...
class CKGConv(nn.Module):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        self.ffn_ratio = cfg.attn.get("ffn_ratio", 1.0)
        ...
        self.mlp_dropout = cfg.attn.get("mlp_dropout", 0.0)
        ...
        if cfg.attn.get("ffn", False):  # FFN-like MLP
            for i in range(num_blocks):
                blocks = blocks + [
                    (
                        nn.Linear(
                            hid_dim, int(hid_dim * self.ffn_ratio), bias=use_bias
                        ),
                        "x -> h",
                    ),
                    ...
                    (
                        nn.Linear(
                            int(hid_dim * self.ffn_ratio), hid_dim, bias=use_bias
                        ),
                        "h -> h",
                    ),
                    (
            ...
            blocks = blocks + [(nn.Linear(hid_dim, hid_dim, bias=use_bias), "x -> x")]
        ...
        if self.value_proj:
            ...
            self.bias = nn.Parameter(
                torch.zeros(1, num_heads, out_dim), requires_grad=True
            )
            self.V.apply(trunc_init_)
        else:
            self.V = nn.Identity()
            self.bias = nn.Parameter(
                torch.zeros(1, num_heads, out_dim), requires_grad=False
            )
    ...
    def propagate_attention(self, batch):
        ...
        if self.blur_kernel:
            ...
            msg = (
                batch.V_h[edge_index[0]] * score
            )  # (num relative) x num_heads x out_dim
            batch.wV = scatter(
                msg, edge_index[1], dim=0, dim_size=batch.num_nodes, reduce="add"
            )
        else:
            score = self.attn_dropout(score)
            msg = (
            ...
            batch.wV = scatter(
                msg, edge_index[1], dim=0, dim_size=batch.num_nodes, reduce="mean"
            )
    ...
    def __init__(
        ...
        **kwargs,
        ...
        if self.layer_norm:
            norm_fn = nn.LayerNorm
        if self.batch_norm:
            norm_fn = partial(nn.BatchNorm1d)

...
class KernelGenerator(nn.Module):
    def __init__(self, cfg=CN()):
        ...
        num_blocks = cfg.attn.get("n_mlp_blocks", 1.0)
        ffn_ratio = cfg.attn.get("ffn_ratio", 1.0)
        ...
        for i in range(num_blocks):
            in_fc = GroupedLinear(
                num_groups, hid_dim, int(hid_dim * ffn_ratio), bias=use_bias
            )
            out_fc = GroupedLinear(
                num_groups, int(hid_dim * ffn_ratio), hid_dim, bias=use_bias
            )
        ...
        if self.deg_scaler:
            ...
            self.deg_coef = nn.Parameter(
                weight * np.sqrt(num_groups), requires_grad=True
            )

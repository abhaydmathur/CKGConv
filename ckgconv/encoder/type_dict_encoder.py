...
@register_edge_encoder("TypeDict2OneHotEdge")
class TypeDict2OneHotEdgeEncoder(torch.nn.Module):
    def __init__(self, emb_dim):
        super().__init__()
        ...
        self.emb = torch.nn.Parameter(torch.eye(num_types), requires_grad=False)
        # torch.nn.init.xavier_uniform_(self.encoder.weight.data)

    def forward(self, batch):
        # batch.edge_attr = self.encoder(batch.edge_attr)
        with torch.no_grad():
            batch.edge_attr = self.emb[batch.edge_attr]

...
class GraphRDBias(nn.Module):
    ...
    def __init__(
        ...
        rd_bias_heads = (
            self.num_heads * self.n_layers if self.no_share else self.num_heads
        )
    ...
    def forward(self, batched_data):
        ...
        res_pos, x, node_type_edge = (
            batched_data["res_pos"],
            batched_data["x"],
        ...
        edge_feature = self.gbf(
            dist,
            (
                torch.zeros_like(node_type_edge).long()
                if node_type_edge is None or self.no_node_feature

...
#
#
# class SyntheticCounting(InMemoryDataset):
#     r"""The SyntheticCounting dataset from the
...
#         assert dataset_type in ['dataset1', 'dataset2']
...
#         return osp.join(self.root, self.task, self.dataset_type, 'processed')
#
#     @property
...
#
#     def process(self):
#         glist, all_labels = load_graphs(osp.join(self.root, self.dataset_type + '.bin'))
#         graphs = []
#         if self.task == 'attributed_triangle': # adding node features according to parity of their node indices
#             for g in glist:
#                 g.ndata['feat'] = torch.FloatTensor([[i % 2, 1 - i % 2] for i in range(g.number_of_nodes())])
...
#             graphs = glist
#
#         labels = all_labels[self.task]
#
#         self.variance = np.std(labels.numpy()) ** 2
#         print("Label variance: ", self.variance)
#
#         for split in ['train', 'val', 'test']:
#             indices = []
#             with open(osp.join(self.root, self.dataset_type + '_{}.txt'.format(split)), "r") as f:
#                 for line in f:
...
#
#             for idx in indices:
#                 curr_graph = graphs[idx]
#                 y = labels[idx]
#                 x = curr_graph.ndata['feat'] if curr_graph.ndata else torch.zeros(curr_graph.number_of_nodes(), 1).long()
#                 edge_index = torch.stack(curr_graph.edges())
#                 edge_attr = curr_graph.edata['feat'] if curr_graph.edata else torch.zeros(curr_graph.number_of_edges()).long()

...
# from .dataset_synthetic_counting import SyntheticCounting
from torch_geometric.graphgym.config import cfg
from torch_geometric.graphgym.loader import load_pyg, load_ogb, set_dataset_attr
from torch_geometric.graphgym.register import register_loader

from ckgconv.loader.dataset.aqsol_molecules import AQSOL
from ckgconv.loader.dataset.coco_superpixels import COCOSuperpixels
from ckgconv.loader.dataset.malnet_tiny import MalNetTiny
from ckgconv.loader.dataset.voc_superpixels import VOCSuperpixels
from ckgconv.loader.split_generator import prepare_splits, set_dataset_splits
from ckgconv.transform.posenc_stats import compute_posenc_stats, ComputePosencStat
from ckgconv.transform.transforms import (
...
def load_dataset_master(format, name, dataset_dir):
    ...
    average_num_nodes(dataset[dataset.data["train_graph_index"]])

    return dataset


def average_num_nodes(dataset):
    """
    Compute average number of nodes per graph in training set
    """
    num_graphs = len(dataset)
    num_nodes = dataset.data.num_nodes
    cfg.dataset.avg_num_nodes = num_nodes / num_graphs

    return cfg.dataset.avg_num_nodes
...
def preformat_OGB_Graph(dataset_dir, name):
    ...
    if name == "ogbg-ppa":
        ...
        from ckgconv.loader.ogbg_code2_utils import (
            idx2vocab,
            get_vocab_mapping,
            augment_edge,
            encode_y_to_arr,
...
def preformat_PCQM4Mv2Contact(dataset_dir, name):
    ...
    try:
        # Load locally to avoid RDKit dependency until necessary
        from ckgconv.loader.dataset.pcqm4mv2_contact import (
...
def preformat_Peptides(dataset_dir, name):
    ...
    try:
        # Load locally to avoid RDKit dependency until necessary.
        from ckgconv.loader.dataset.peptides_functional import PeptidesFunctionalDataset
        from ckgconv.loader.dataset.peptides_structural import PeptidesStructuralDataset
    except Exception as e:
        logging.error(

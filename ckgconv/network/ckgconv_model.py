...
class Stem(torch.nn.Module):
    ...
    def __init__(self, dim_in, **kwargs):
        ...
        if edge_stem is None:
            edge_stem = ""
        edge_stem = edge_stem.split("+")
        self.edge_stem = nn.Sequential(
            *[
                register.edge_encoder_dict[enc](
                    out_dim=cfg.gnn.dim_inner,
                )
                for enc in edge_stem
                if enc != ""
            ]
        )
...
class CKGraphConvNet(torch.nn.Module):
    ...
    def __init__(self, dim_in, dim_out):
        ...
        norm_fn = nn.Identity
        if cfg.gt.batch_norm:
            norm_fn = partial(nn.BatchNorm1d, momentum=cfg.gt.bn_momentum)
        ...
        if pre_backbone_norm:
            self.pre_backbone_norm = NormalizationLayer(
                norm_fn(cfg.gt.dim_hidden), graph_norm=graph_norm
            )
        else:
        ...
        if post_backbone_norm:
            self.post_backbone_norm = NormalizationLayer(
                norm_fn(cfg.gt.dim_hidden), graph_norm=graph_norm
            )
        else:

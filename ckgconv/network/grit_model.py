    ...
    def __init__(self, dim_in, **kwargs):
        ...
        self.abs_encoder = nn.Sequential(
            *[
                ...
                for enc in node_pe_encoder
                if enc != ""
            ]
        )

        edge_pe_encoder = cfg.gt.get("edge_pe_encoder", None)
        if edge_pe_encoder is None:
            edge_pe_encoder = ""
...
class GritTransformer(torch.nn.Module):
    ...
    def __init__(self, dim_in, dim_out):
        ...
        if (
            global_model_type == "Norm-Res-GritTransformer"
            or global_model_type == "PreNormGritTransformer"
        ):
            layers.append(
                register.layer_dict["GeneralNormLayer"](

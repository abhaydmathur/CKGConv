...
def plateau_scheduler(
    optimizer: Optimizer, patience: int, lr_decay: float
) -> ReduceLROnPlateau:
...
def scheduler_reduce_on_plateau(
    ...
    if train_mode == "standard":
        raise ValueError(
            "ReduceLROnPlateau scheduler is not supported "
...
def linear_with_warmup_scheduler(
    optimizer: Optimizer, num_warmup_epochs: int, max_epoch: int
):
